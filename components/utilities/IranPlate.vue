<template>
  <div
    class="w-100 pa-2 rounded-lg justify-center d-flex flex-wrap"
    style="border: 2px dashed rgba(0, 0, 0, 0.2)"
  >
    <h2 class="font-weight-bold">پلاک ایران</h2>
    <div class="col-12 custom-plate">
...
export default {
  ...
  created() {
    if (typeof this.vehicleNumber !== 'string') return

    const first = this.vehicleNumber.substr(0, 2)
    const letter = this.vehicleNumber.substr(2, 3)
    const second = this.vehicleNumber.substr(5, 3)
    const third = this.vehicleNumber.substr(8, 2)
